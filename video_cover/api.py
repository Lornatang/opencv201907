...
SAVE_DIR = "./dlcv/cc/video_cover"


class CCVideoCover(APIView):
    ...
    def post(request):
        ...
        try:
            if url:
                # download image
                os.system(f"./dlcv/cc/video_cover/VideoCover {url}")
            else:
        ...
        try:
            image_path = os.path.join(SAVE_DIR, "smile.png")
            ...
            ret["status_code"] = 1004
            ret["message"] = "图片编码异常"
            ret["image"] = None
            return JsonResponse(ret)
        os.remove("./dlcv/cc/video_cover/video.mp4")
        return JsonResponse(ret)
